  ...
  return(
    <div className={cl("events-tabs", "tabs")}>
      <div className={cl("events-tabs__controls-wraper")}>
        <Search toggleEventsDate={[setStartEventsDate,setEndEventsDate]}/>
        <ul className={cl("tabs-controls")}>
          {
            events.map( item => {
                return(
                  <li className={cl({"tabs-controls__item--active": item.city.name_en === activeCity ? true : false,
                  }, "tabs-controls__item")}
        ...
        <div className={cl("tabs-content", "events-tabs__content-wraper")}>
          {events.map( item => {
              return (
                <div className={cl({"tabs-content__item--active": item.city.name_en === activeCity ? true : false},
                "tabs-content__item")} key={item.city.name_en}>
                  {item.items !== undefined ?
                    item.items.map( (event, index) => {
                      checkEventDate(event.eventDate);
                      if (showEvent) {
                        return (
                          <Meropriyatiya_item  {...event} key={index}/>
                        )
                      }
                    }):
